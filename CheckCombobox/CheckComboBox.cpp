	...
	switch (nMsg) {
		...
		case WM_CHAR: {
			if (wParam == VK_SPACE) {
				...
				m_pComboBox->GetParent()->SendMessage(WM_COMMAND, MAKELONG(GetWindowLong(m_pComboBox->m_hWnd, GWLP_ID), CBN_SELCHANGE), (LPARAM)m_pComboBox->m_hWnd);
				return 0;
			}
		...
		case WM_LBUTTONDOWN: {
			...
			if (PtInRect(rcClient, pt)) {
				...
				if (PtInRect(rcItem, pt)) {
					...
					// Notify that selection has changed
					m_pComboBox->GetParent()->SendMessage(WM_COMMAND, MAKELONG(GetWindowLong(m_pComboBox->m_hWnd, GWLP_ID), CBN_SELCHANGE), (LPARAM)m_pComboBox->m_hWnd); // GWLP_WNDPROC
